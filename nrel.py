...
ALL_STATES_FILE = 'all_states_emissions_recent'
ALL_STATES_CSV = '%s.csv' % ALL_STATES_FILE
ALL_STATES_HTML = '%s.html' % ALL_STATES_FILE
...
class NRELClient(object):
    ...
    def write_all_states_emissions_recent_sorted(self):
        ...
        with open(ALL_STATES_CSV, 'w') as outfile:
    ...
    def write_all_states_emissions_html(self):
        if not os.path.isfile(ALL_STATES_CSV):
            self.write_all_states_emissions_recent_sorted()

        with open(ALL_STATES_CSV, 'r') as infile:
            reader = csv.reader(infile)
            with open('vert_%s' % ALL_STATES_HTML, 'w') as vert_file:
                with open('horiz_%s' % ALL_STATES_HTML, 'w') as horiz_file:
                    for state in reader:
                        vertical_bar_html = '''
...
if __name__=='__main__':
    client = NRELClient()
    client.write_all_states_emissions_html()
